...
/**
 * @brief Magneto-Inductive PHY: analytical AWGN PER model for BPSK.
 */
...
protected:
    ...
    double AcquisitionThreshold_dB_ = 10.0;   // SNR gate [dB]
    int    use_auto_rx_power_gate_  = 1;      // if 1, Pthr = N + SNRreq [dB]

    // Used as dBm in comparisons in startRx()
    double rxPowerThreshold_dBm_    = -140.0; // dBm fallback gate

    double Rb_ = 1000.0;        // bps
    double B_  = 1000.0;        // Hz (mask width)

    double Beff_Hz_ = 10000.0;  // Hz (bound, but Beff is computed from B_ and resonance)
